...
test.describe('Form Layouts page', () => {
    ...
    test.skip('input fields', async ({ page }) => {
        ...
        // await usingTheGridEmailInput.pressSequentially('test2@test.com', { delay: 500 }) //slow type
    ...
    test.skip('Radio buttons', async ({ page }) => {
        ...
        //force to check hidden elements
...
test.skip('checkbox', async ({ page }) => {
    ...
    //conver a list into an array and check all checkboxes & assertion
    for (const box of await allBoxes.all()) {
        await box.check({ force: true });
        expect(await box.isChecked()).toBeTruthy();

    }
    // await page.pause(); // Pause to inspect the state of checkboxes
})
...
test('list and dropdowns', async ({ page }) => {
    ...
    await dropdownMenu.click(); //close the dropdown menu
...
test('Dialogs box', async ({ page }) => {
    ...
    //page on does not work with dialogs, so we need to use page.on('dialog')
    //tr are table rows and td are table data 
...
test('web tables', async ({ page }) => {
    ...
    // the locators sometimes can change from html data to input
    await page.getByText('Tables & Data').click();
    await page.getByText('Smart Table').click();
    ...
    await page.locator('input-editor').getByPlaceholder('First Name').fill('Emma');
    ...
    await page.locator('input-editor').getByPlaceholder('E-mail').fill('pepe@testing.com');
    await page.locator('.nb-checkmark').click();
    await expect(targetRowById.locator('td').nth(5)).toHaveText('pepe@testing.com')
...
test('Sliders', async ({ page }) => {
    ...
    //placing our coordinates in the center of the box
