...
test('User facing locators', async ({ page }) => {
    ...
    // Es necesario agregarlo al source code 
    // await page.getByTestId('Sign In') 
   
})

test('loccating child elements', async ({ page }) => {
...
test('Reusing test locators', async ({ page }) => {
    const emailValue = "test@test.com"
    const basicForm = page.locator('nb-card').filter({ hasText: "Basic form" })
    const basicForm2 = page.locator('nb-card').getByText( "Basic form" )
